...
function filterStudentsByGrade(students, grade) {
  ...
  filteredStudents.forEach((student) => {
    console.log(`Студент ${student.name} отримав оцінку ${student.grade}`);
  });
}
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      ...
      // colMatrix[j] = Math.floor(Math.random() * 10);
      rowMatrix[j] = Math.floor(Math.random() * 10);
    }
...
function traceBackward(n) {
  ...
  if (n > 1) {
    ...
    // Коли доходимо до 1, виводимо стек викликів.
...
function displayGroupedInfo(students) {
  ...
  for (let student of students) {
    ...
    console.groupCollapsed(`  Вік: ${student.age}`);
    console.groupCollapsed(`Предмет: ${student.subject}`);
    console.groupEnd();
    console.groupEnd();
    console.groupEnd();
  }
...
function validateUserInput(userInput) {
  ...
  console.assert(userInput.password != null, "Помилка: пароль відсутній!");
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word.toLowerCase()) {
    // Перевіряємо, чи є літера в нашому рядку з голосними
    if (vowels.includes(letter)) {
      // Якщо так, збільшуємо лічильник голосних
      vowelsCount++;
    } else if (/^[a-z]$/.test(letter)) {
      // Якщо літера - буква і не є голосною, збільшуємо лічильник приголосних
      consonantsCount++;
    }
  }
